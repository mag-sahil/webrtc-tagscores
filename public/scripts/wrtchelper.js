var WrtcHelper = (function () {
    ...
    function eventBinding() {
        ...
        $("#btnMuteUnmute").on('click', async function () {
            ...
            voice_status=$(".Enable_mute_"+candidate_id).attr("data-status");
    ...
    async function RemoveAudioVideoSenders(rtpSenders) {
            for (var con_id in peers_con_ids) {                    
                 if($(".Enable_mute_"+con_id).hasClass("fa-microphone"))
                 {
                    $(".Enable_mute_"+con_id).removeClass("fa-microphone");
                    $(".Enable_mute_"+con_id).addClass("fa-microphone-slash"); 
                 }
                  $('.Enable_mute_'+con_id).attr("data-status",true);
    ...
    async function AddUpdateAudioVideoSenders(track,rtpSenders) {
        for (var con_id in peers_con_ids) {
             if(con_id==candidate_id){
                if($(".Enable_mute_"+con_id).hasClass("fa-microphone-slash"))
                 {
                    $(".Enable_mute_"+con_id).removeClass("fa-microphone-slash");
                   $(".Enable_mute_"+con_id).addClass("fa-microphone"); 
                 }
            $('.Enable_mute_'+con_id).attr("data-status",false);
    ...
    function closeConnection(connid) {
        ...
        { 
           ...
           //const blob = new f(recordedBlobs, {type: 'video/webm'});
           // const url = window.URL.createObjectURL(blob);

           var curBuffer = new Buffer( blob, 'binary' );
    ...
    body: JSON.stringify({ data: {
          ...
          // socket.emit('download_recording',{data:fileObject});
         ...
         function uploadRecordedVideo() {
           ...
           var blob = new Blob(recordedBlobs, {type: 'text/plain'});
           ...
           formData.append('schedule_id', 50);
           var upload_url = "upload_data.php";
     ...
     function getFileName(fileExtension) {
       ...
       return '<?php echo $prcanid; ?>-Video-' + year + month + date + '-' + getRandomString() + '.' + fileExtension;
   }
